...
# Data from Protected Planet at <https://www.protectedplanet.net>
pa <- rbind(
  read_sf("data/protected_areas/WDPA_WDOECM_Sep2022_Public_BRA_shp_0"),
  ...
  transmute(name = NAME, designation = DESIG_ENG, type = GOV_TYPE,
    # One was designated by the Romans, we cap it at the interwar period
    year = as.integer(pmax(1926, STATUS_YR)), pa_area_km2 = REP_AREA) |>
...
pa_df <- pa_int |>
  ...
  rename(pa_fed_km2 = `Federal or national ministry or agency`,
    pa_ind_km2 = `Indigenous peoples`, pa_oth_km2 = `Not Reported`,
    pa_sub_km2 = `Sub-national ministry or agency`,
    pa_loc_km2 = `Local communities`) |>
  ungroup()

pa_int <- pa_int |> arrange(muni_id, pa_year)
pa_tot <- c()
for(m_id in unique(pa_int$muni_id)) {
  temp <- pa_int |> filter(muni_id == m_id)
  if(nrow(temp) > 1) {
    for(yy in seq(2, nrow(temp))) {
      temp[yy, "geometry"] <- st_make_valid(st_set_precision(
        st_union(temp[yy - 1, "geometry"], temp[yy, "geometry"]), 1e6))
    }
  }
  temp <- temp |>
...
muni_grid <- expand.grid("muni_id" = unique(sh$muni_id), "pa_year" = seq(min(pa_df$pa_year), 2020)) |>
...
pa_df <- left_join(muni_grid, pa_df |> select(-muni, -state, -area_km2), by = c("muni_id", "pa_year")) |>
  tidyr::replace_na(list(pa_fed_km2 = 0, pa_ind_km2 = 0, pa_oth_km2 = 0, pa_sub_km2 = 0, pa_loc_km2 = 0)) |>
  arrange(muni_id, pa_year) |>
  group_by(muni_id) |>
  mutate(across(pa_fed_km2:pa_loc_km2, ~cumsum(.x)), pa_tot_km2 = zoo::na.locf(pa_tot_km2, na.rm = FALSE)) |>
  tidyr::replace_na(list(pa_tot_km2 = 0)) |>
  ungroup() |>
  rename(year = pa_year)

# Save a dataframe to work with
