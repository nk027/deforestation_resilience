...
d <- full_join(readRDS("data/spatial.rds"), 
               readRDS("data/land_use_change.rds"), 
               by = c("muni", "state"))

d <- d |> left_join(readRDS("data/fines.rds"),
                    by = c("muni", "state", "year")) |>
  tidyr::replace_na(list(n_fined = 0, brl_fined = 0, n_payments = 0, brl_paid = 0,
                         n_cancelled = 0, brl_cancelled = 0,
                         n_outlier_cancelled = 0, brl_outlier_cancelled = 0))
